---
title: 'React Diff'
excerpt: 在react中，更新`UI`必须生成新的`JSX`对象（称之为`visual DOM`），而`新的JSX`对象和`老的visual DOM`之间对比以最小的操作次数来获取`差异节点`（也称之为副作用），从而高效的更新`UI`。
coverImage: '/assets/blog/react/cover/diff.jpg!s2'
