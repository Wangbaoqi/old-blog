---
title: 'Browser 架构'
excerpt: '想要构建高性能的Web应用程序(网络流程、渲染流程、JavaScript执行以及Web安全),必然离不开浏览器。因此，掌握浏览器是非常重要的。'
coverImage: '/assets/blog/browser/cover/frame.webp'
date: '2021-04-16'
author:
  name: NateWang
...
import Link from './mdx/component/link.js'

想要构建高性能的**Web**应用程序(网络流程、渲染流程、**JavaScript**执行以及Web安全),必然离不开浏览器。因此，掌握浏览器是非常重要的。

export const link = {
  web: 'https://developer.mozilla.org/zh-CN/docs/Glossary/World_Wide_Web',
  browserDe: 'https://zh.wikipedia.org/wiki/%E7%BD%91%E9%A1%B5%E6%B5%8F%E8%A7%88%E5%99%A8',
  hyperLink: 'https://developer.mozilla.org/zh-CN/docs/Glossary/Hyperlink',
}

export const cBox = (
  <>
    <Para>
      网页浏览器是一种从<Link href={link.web} val='Web'/>获取和显示页面的程序，并且让用户通过 <Link href={link.hyperLink} val='超链接'/>访问更多页面。 
      <Strong>维基百科</Strong>也给出了网页浏览器的<Link href={link.browserDe} val='定义'/>。
    </Para>
    <Para>
      浏览器有很多种，而使用最多的是Chrome 浏览器，除此之外，还有<Strong>Safari</Strong>, <Strong>IE</Strong>, <Strong>FireFox</Strong>, <Strong>FireFox</Strong>浏览器等。
    </Para>
    <Para>
      浏览器最重要的就是其内核，也是<Strong>渲染引擎</Strong>，用来解释网页语法并且将其渲染到页面，不同的浏览器其解释语法也不一样，因此，部分语法需要兼容多个浏览器。
    </Para>
    <Para>
      浏览器内核除了渲染引擎之外，还包括了<Strong>JS引擎</Strong>，不过现在JS引擎越来越独立了，内核就更倾向渲染引擎了，比如，著名的<Strong>Google V8引擎</Strong>。
    </Para>
  </>
)


<SideNote type='info' title='那什么是浏览器' children={cBox}/>
...
![](/assets/blog/browser/browser-m.webp)
...
![](/assets/blog/browser/browser-e.webp)
...
![](/assets/blog/browser/browser-n.webp)
...
![](/assets/blog/browser/browser-p.webp)
...
![](/assets/blog/browser/browserui.webp)
...
![](/assets/blog/browser/browser-s.webp)
