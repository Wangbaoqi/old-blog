...
export const getFeaturePost = async () => {
  ...
  return featurePosts.slice(0, 10);
}
...
export const getGroupByCategory = async (type = 'category') => {
  const allPosts = await getAllPosts();
  const cateGoryList =  allPosts.filter(post => !post.postShow && post.category)
...
export const getallSnippets = async () => {
  const allPosts = await getAllPosts();
  return allPosts.filter(post => !post.postShow && post.snippet)
}

export const getGroupBySnippets = async (type = 'snippet') => {
  const allPosts = await getallSnippets();
...
export const getAlgorithmEvery = async (limit = 15) => {
  const allPosts = await getAllPosts();
  const dayLists = allPosts.filter(post => post['subCategory'] == 'everyDay').sort((a, b) => a.id < b.id);
  return dayLists.slice(0, limit);
}

export const getAlgorithmEveryByType = async (tagType) => {
  const allPosts = await getAllPosts();
  return allPosts.filter(post => post['subCategory'] == 'everyDay' && post.tags.includes(tagType)).sort((a, b) => a.id < b.id);
}
