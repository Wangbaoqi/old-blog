...
export default function FeaturePost({
  ...
  const rightContent = (
    <div className='px-2 py-6 md:py-4 md:px-8 relative flex flex-col justify-between '>
      <section>
        <h3 className="md:text-lg text-base  leading-snug mb-4 md:mb-7 dark:text-white group-hover:text-second-color">
  ...
  const bottomContent = (
    <div className='p-3 relative '>
      <div className='my-4 flex items-center '>
        <Avatar name={author.name} picture={author.picture} />
        <span className='text-xs  h-5 flex items-center'>{date}</span>
      </div>
    ...
    <article className={`cursor-pointer md:min-h-f-card group overflow-hidden ${bgCls}`}>
      <div className={`md:flex ${postCls}`}>
        <div className="relative md:shrink-0">
          ...
          <span className={`text-pre  rounded absolute top-4 left-4 px-2 py-1 cursor-pointer ${tagTheme[category]}`}>{ category }</span>
        </div>
